...
export default function Menu() {
  ...
  const accountLabel = user?.reloadUserInfo?.screenName
    ? `@${String(user.reloadUserInfo.screenName).replace(/^@+/, '')}`
    : user?.displayName || user?.email || 'GitHub pilot';
  const personalBest = highScores[0] || 0;
    ...
    >
      <div className="glass-panel p-5 text-center">
        ...
        <p className="lead-mono mb-5">
          Quantum core requires manual stabilization.
          <br />
        ...
        {showScores && (
          ...
          >
            <div className="eyebrow mb-2">PERSONAL BEST</div>
            {personalBest === 0 ? (
              <div className="text-muted-mono">NO RECORD YET</div>
            ) : (
              <div className="score-card">
                <div className="score-card__headline">BEST RUN</div>
                <div className="score-card__value">{personalBest}</div>
              </div>
            )}
              ...
              {leaderboardLoading ? (
                ...
                <ol className="scores-list">
                  {leaderboard.map((entry, index) => (
                    <li key={entry.id} className="scores-list__row">
                      <span className="scores-list__label">
                        <span className="scores-list__rank">#{index + 1}</span>
                        {entry.profileUrl ? (
                          <a
